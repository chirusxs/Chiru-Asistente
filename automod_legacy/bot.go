...
func CheckMessage(evt *eventsystem.EventData, m *discordgo.Message) bool {
	...
	go func() {
		...
		case PunishNone:
			err = moderation.WarnUser(nil, cs.GuildID, cs, m, common.BotUser, &member.User, ""+punishMsg)
		case PunishMute:
			err = moderation.MuteUnmuteUser(nil, true, cs.GuildID, cs, m, common.BotUser, ""+punishMsg, member, muteDuration)
		case PunishKick:
			err = moderation.KickUser(nil, cs.GuildID, cs, m, common.BotUser, ""+punishMsg, &member.User, -1)
		case PunishBan:
			err = moderation.BanUser(nil, cs.GuildID, cs, m, common.BotUser, ""+punishMsg, &member.User)
		}
